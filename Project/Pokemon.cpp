...
{  
    for(int i = 0; i < key.size(); i++) //make key all lowercase - helpful for searching
    {
        key[i] = tolower(key[i]); 
    }
    
    int hash = 0; 
    for (int i = 0; i < key.size(); i++) //add up all the ascii values
    {
        hash = hash + int(key[i]);
    }
    return (hash%tableSize); //modulate by table size
}

void PokeHash::sort(pokeNode *pokemon) //sorts pokemon into respective type vectors
{
...
//returns false when the vector size is 0 so that user cannot pick
...
void PokeHash::printPokemonByTypes()
{
    ...
    cout << endl;
    if(!printByType(stoi(type)))//checks if vector is empty or not
    {
        return;
    }
    string idx; 
